    ...
    module Translator
      class << self
        ...
        def translate_from_file(file_path, options = {})
          ...
          set_options(route, options)
          r.translate Rails.application.routes
        end

        def i18n(options = {}, *locales)
          route = RailsTranslateRoutes.init_with_i18n(*locales)
          set_options(route, options)
          route.translate Rails.application.routes
        end

        private
        def set_options(route, options)
          route.prefix_on_default_locale = true if options && options[:prefix_on_default_locale] == true
          route.no_prefixes = true if options && options[:no_prefixes] == true
          route.keep_untranslated_routes = true if options && options[:keep_untranslated_routes] == true
        end
      end
    end
  end
